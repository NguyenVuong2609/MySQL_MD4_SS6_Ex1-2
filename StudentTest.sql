-- create database test_student_management_ex6;
use test_student_management_ex6;
create table Student (
RN int primary key,
sName varchar(20),
sAge tinyint
);
create table Test (
testID int primary key,
tName varchar(20)
);
create table StudentTest (
RN int,
testID int,
date datetime,
mark float
);
alter table StudentTest add foreign key (RN) references Student(RN);
alter table StudentTest add foreign key (testID) references Test(testID);

insert into Student values
(1,"Nguyen Hong Ha", 20),
(2, "Truong Ngocj Anh", 30),
(3, "Tuan Minh", 25),
(4, "Dan Truong", 22);

insert into Test values
(1, "EPC"),
(2, "DWMX"),
(3, "SQL1"),
(4, "SQL2");

insert into StudentTest values
(1,1,"2006-07-17",8),
(1,2,"2006-7-18",5),
(1,3,"2006-07-19",7),
(2,1,"2006-07-17",7),
(2,2,"2006-07-18",4),
(2,3,"2006-07-19",2),
(3,1,"2006-07-17",10),
(3,3,"2006-07-18",1);

-- Thêm ràng buộc dữ liệu cho cột age với giá trị thuộc khoảng: 15-55
alter table student add constraint sAge check(sAge between 15 and 55);

-- Thêm giá trị mặc định cho cột mark trong bảng StudentTest là 0
alter table studenttest modify column mark float default(0);

-- Thêm khóa chính cho bảng studenttest là (RN,TestID)
alter table studenttest add primary key(RN,TestID);

-- Thêm ràng buộc duy nhất (unique) cho cột name trên bảng Test
alter table test modify column tName varchar(20) unique;

-- Xóa ràng buộc duy nhất (unique) trên bảng Test
alter table test drop index tName;

-- Hiển thị danh sách các học viên đã tham gia thi, các môn thi được thi bởi các học viên đó
select row_number() over (order by s.sName ASC) as 'STT',s.sName as "Student Name", t.tname as "Test Name", st.mark as "Mark", st.date as "Date"
from studenttest st
join student s on s.RN = st.RN
join test t on t.testID = st.testID;

-- Hiển thị danh sách các bạn học viên chưa thi môn nào
select s.RN, s.sName, s.sage as "Age"
from student s
where not exists (select s.RN from student join studenttest where studenttest.RN = s.RN);

-- Hiển thị danh sách học viên phải thi lại, tên môn học phải thi lại và điểm thi(điểm phải thi lại là điểm nhỏ hơn 5)
select s.sname as "Student Name", t.tname as "Test Name", st.mark as "Mark", st.date as "Date"
from studenttest st
join student s on s.RN = st.RN
join test t on t.testID = st.testID
where st.mark < 5;

-- Hiển thị danh sách học viên và điểm trung bình(Average) của các môn đã thi. Danh sách phải sắp xếp theo thứ tự điểm trung bình giảm dần
select s.sname as "Student Name", avg(st.mark) as Average
from student s
join studenttest st on s.RN = st.RN
group by s.sname
order by Average DESC;

-- Hiển thị tên và điểm trung bình của học viên có điểm trung bình lớn nhất
select s.sname as "Student Name", avg(st.mark) as Average
from student s
join studenttest st on s.RN = st.RN
group by s.sname
order by Average DESC
limit 1;

-- Hiển thị điểm thi cao nhất của từng môn học
select t.tname as "Test Name", max(st.mark) as "Max Mark"
from test t
join studenttest st on t.testID = st.testID
group by t.testID
order by t.tname;

-- Hiển thị danh sách tất cả các học viên và môn học mà các học viên đó đã thi nếu học viên chưa thi môn nào thì phần tên môn học để Null
select s.sname as "Student Name", t.tname as "Test Name"
from student s 
left join studenttest st on s.RN = st.RN
left join test t on t.testID = st.testID;

-- Sửa (Update) tuổi của tất cả các học viên mỗi người lên một tuổi
update student set sAge = (sAge + 1);

-- Thêm trường tên là Status có kiểu Varchar(10) vào bảng Student
alter table student add column sStatus varchar(10);	

-- Cập nhật(Update) trường Status sao cho những học viên nhỏ hơn 30 tuổi sẽ nhận giá trị ‘Young’, trường hợp còn lại nhận giá trị ‘Old’ sau đó hiển thị toàn bộ nội dung bảng Student
update student s set sStatus = if(s.sAge<30,"Young","Old");
select * from student;

-- Hiển thị danh sách học viên và điểm thi, dánh sách phải sắp xếp tăng dần theo ngày thi
select s.sName as "Student Name", t.tName as "Test Name", st.mark as Mark, st.Date 
from studenttest st
join student s on s.RN = st.RN
join test t on t.testID = st.testID
order by st.Date ASC;

-- Hiển thị các thông tin sinh viên có tên bắt đầu bằng ký tự ‘T’ và điểm thi trung bình >4.5. Thông tin bao gồm Tên sinh viên, tuổi, điểm trung bình
select s.sName, s.sAge, avg(st.mark) as Average
from student s
join studenttest st on s.RN = st.RN
where s.sName like "T%"
group by s.RN
having Average > 4.5;

-- Hiển thị các thông tin sinh viên (Mã, tên, tuổi, điểm trung bình, xếp hạng). Trong đó, xếp hạng dựa vào điểm trung bình của học viên, điểm trung bình cao nhất thì xếp hạng 1
select s.RN, s.sName, s.sAge, avg(st.mark) as Average, row_number() over (order by avg(st.mark) DESC) as 'Rank'
from student s
join studenttest st on s.RN = st.RN
group by s.RN;

-- Sửa đổi kiểu dữ liệu cột name trong bảng student thành nvarchar(max)
alter table student modify column sName varchar(255);

-- Cập nhật (sử dụng phương thức write) cột name trong bảng student với yêu cầu sau: Nếu tuổi >20 -> thêm ‘Old’ vào trước tên (cột name)Nếu tuổi <=20 thì thêm ‘Young’ vào trước tên (cột name)
update student s set s.sName = concat(if(s.sAge>20,"Old ","Young "),s.sname);

-- Xóa tất cả các môn học chưa có bất kỳ sinh viên nào thi
-- SELECT DISTINCT test.testId AS tid FROM test JOIN studenttest ON studenttest.testID = test.testid;

delete  FROM test t1
  WHERE NOT EXISTS (SELECT 1 FROM studenttest t2 WHERE t1.TestID = t2.TestID);
    
    
-- Xóa thông tin điểm thi của sinh viên có điểm <5
delete from studenttest st where st.mark < 5;